...
> Первый адрес Coils-таблицы (`COIL_FILTER_TIMER_RESET`=1) сам по себе не используется как
> выключатель — согласно заданию, «первый адрес — standby mode — отключение» реализован через
> запись `0` в Holding-регистр `HR_USER_CONFIG_CURRENT_SYSTEM_MODE` (адрес 1), т.к. именно там
> закодированы состояния `0:Standby, 1:Building protection, 2:Economy, 3:Comfort`.
...
## Важные примечания по адресации

* В файле `const.py` все адреса регистров указаны **как в Modbus-таблице производителя**
  (1-based). Клиент (`modbus_client.py`) автоматически вычитает 1 при обращении к
  pymodbus (0-based протокол), поэтому в UI/логах адреса совпадают с документацией
  на контроллер.
* Отрицательные значения (например, `-1: Auto` для рециркуляции) обрабатываются как
  знаковые 16-битные регистры (two's complement).
* Значения температур хранятся в регистрах как целые числа ×10 (например, `210` = `21.0 °C`).
...
```bash
cd rekuperator_modbus
git init
git add .
git commit -m "Initial commit: Rekuperator MCB Modbus HACS integration"
git branch -M main

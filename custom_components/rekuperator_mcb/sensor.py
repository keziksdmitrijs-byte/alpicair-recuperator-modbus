...
class RekuperatorSensorDescription(SensorEntityDescription):
    """Describes a Rekuperator sensor sourced from a specific register table."""

    register_group: str = "input"  # "input" | "holding" | "coils"
    address: int = 0
    scale: float = 1.0
...
SENSOR_DESCRIPTIONS: tuple[RekuperatorSensorDescription, ...] = (
    ...
    # Actual air flow per stage - extract
    RekuperatorSensorDescription(
    ...
    # Actual air flow per stage - supply
    RekuperatorSensorDescription(
...
class RekuperatorSensor(CoordinatorEntity[RekuperatorDataUpdateCoordinator], SensorEntity):
    """Representation of a read-only Rekuperator sensor."""

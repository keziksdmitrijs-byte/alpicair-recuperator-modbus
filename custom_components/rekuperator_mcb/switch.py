...
Two switches share the "coils" table:

* Night cooling (COIL_NIGHT_COOLING_FUNCTION, address 4) - a persistent
  Enabled/Disabled flag, read back from the coil itself.
* Standby / power (HR_USER_CONFIG_CURRENT_SYSTEM_MODE, address 1) - the
  first Holding register. Value 0 = Standby (unit OFF), any of
  1/2/3 = running in some mode. Turning this switch back ON restores the
  previously selected mode (Comfort by default) rather than a fixed mode,
  matching how the physical remote's "OnOff" behaves.
"""
...
class RekuperatorNightCoolingSwitch(_BaseRekuperatorSwitch):
    """ON/OFF switch for COIL_NIGHT_COOLING_FUNCTION (coils address 4)."""
...
class RekuperatorStandbySwitch(_BaseRekuperatorSwitch):
    """Main power switch, mapped to HR_USER_CONFIG_CURRENT_SYSTEM_MODE (address 1).

    OFF writes 0 (Standby). ON restores Comfort mode (value 3) unless the
    unit is already in Building protection/Economy/Comfort, in which case
    turning on again is a no-op.
    """

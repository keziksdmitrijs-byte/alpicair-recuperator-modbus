...
Includes the writable Holding-register parameters requested:
 - Night cooling schedule/thresholds (HR 25-32)
 - Actual air flow steps 1-4 for supply/extract (HR 450-459)
 - A single "Целевая температура" entity that always writes to the
   Holding register of the *currently active* system mode
   (Building protection=6, Economy=4, Comfort=2), per
   MODE_TO_TEMPERATURE_REGISTER in const.py.
"""
...
NUMBER_DESCRIPTIONS: tuple[RekuperatorNumberDescription, ...] = (
    ...
    # --- Actual air flow steps (fan curve, %) ---
    RekuperatorNumberDescription(
...
class RekuperatorNumber(_BaseRekuperatorNumber):
    """Generic writable Holding-register number entity."""
...
class RekuperatorTargetTemperatureNumber(_BaseRekuperatorNumber):
    """Единая сущность 'Целевая температура'.

    Читает/пишет значение из/в регистр Holding register, соответствующий
    ТЕКУЩЕМУ выбранному режиму работы (Building protection=HR6,
    Economy=HR4, Comfort=HR2), согласно MODE_TO_TEMPERATURE_REGISTER.
    Если контроллер в режиме Standby, запись не выполняется - сначала
    нужно выбрать рабочий режим через select.rekuperator_mode.
    """
    ...
    def native_value(self) -> float | None:
        ...
        if raw is None or raw < 0:
            # 0 / negative means "Energy saving" for Economy/Building
            # protection modes; report as unknown rather than a bogus temp.
            return None

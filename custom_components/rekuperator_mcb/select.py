...
select.rekuperator_mode exposes four options:
  - "Защита здания"      -> writes HR_USER_CONFIG_CURRENT_SYSTEM_MODE = 1
  - "Эконом"              -> writes HR_USER_CONFIG_CURRENT_SYSTEM_MODE = 2
  - "Комфорт"             -> writes HR_USER_CONFIG_CURRENT_SYSTEM_MODE = 3
  - "Интенсивный обдув"   -> pulses COIL_INTENSIVE_AIR_FLOW_BOOST (coils
                              address 5). This coil is a momentary
                              "Activate" trigger (per the Modbus table:
                              "0: Nothing, 1: Activate"), it does not
                              persist as a system mode. Selecting it in HA
                              triggers the boost; the select entity's
                              displayed state falls back to the actual
                              current mode reported by the controller on
                              the next poll.
"""
...
class RekuperatorModeSelect(CoordinatorEntity[RekuperatorDataUpdateCoordinator], SelectEntity):
    """select.rekuperator_mode - system mode + intensive boost trigger."""

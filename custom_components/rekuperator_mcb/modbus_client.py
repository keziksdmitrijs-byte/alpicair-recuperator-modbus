...
Addresses in const.py are 0-based, matching the AlpicAir/NEST Modbus table.
...
class RekuperatorModbusClient:
    ...
    async def read_holding_register(self, address: int) -> int | None:
        """Read a single holding register. Address is 0-based (AlpicAir/NEST table)."""
    ...
    async def write_holding_register(self, address: int, value: int) -> bool:
        """Write a single holding register. Address is 0-based (AlpicAir/NEST table)."""
        await self._ensure_connected()
        if value < 0:
            value += 0x10000
        try:
    ...
    async def read_coil(self, address: int) -> bool | None:
        """Read a single coil. Address is 0-based (AlpicAir/NEST table)."""
        await self._ensure_connected()
        try:
    ...
    async def write_coil(self, address: int, value: bool) -> bool:
        """Write a single coil. Address is 0-based (AlpicAir/NEST table)."""
        await self._ensure_connected()
        try:
    ...
    async def read_input_register(self, address: int) -> int | None:
        """Read a single input register. Address is 0-based (AlpicAir/NEST table)."""
        await self._ensure_connected()
        try:
